...
(function(html) {
    ...
    * ------------------------------------------------------ */
    ...
    const ssTeamScrollAnimation = function() {
    ...
    }; // end ssTeamScrollAnimation
    
    document.addEventListener('DOMContentLoaded', function () {
        const textContainer = document.getElementById('typing-text');
        const spans = textContainer.querySelectorAll('span'); // Get all spans
        const typingSpeed = 50; // Speed per character in milliseconds
        const lineDelay = 500; // Delay between typing each line
        ...
        function typeLine() {
            if (lineIndex < spans.length) {
                ...
                const text = currentSpan.getAttribute('data-text');
    
                if (charIndex < text.length) {
                    currentSpan.innerHTML += text.charAt(charIndex);
                    charIndex++;
        ...
        // Trigger typing animation when the section is visible
        const observer = new IntersectionObserver(
            (entries) => {
                entries.forEach((entry) => {
                    if (entry.isIntersecting) {
                        typeLine();
                        observer.unobserve(textContainer); // Stop observing after animation starts
                    }
                });
            },
